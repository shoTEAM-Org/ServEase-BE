...
function confidence(
  input: CalculatePricingQuoteInput,
  distanceKm: number,
): PricingConfidence {
  // Confidence: downgrade if critical data is missing or distance is zero/base (ambiguous)
  if (
    !input.laborBaseline ||
    input.providerBaseMissing ||
    input.fuel.freshness === 'default'
  ) {
    return 'low';
  }

  // If fuel is stale (> 3 days), downgrade slightly
  if (input.fuel.freshness === 'stale') {
    return 'medium';
  }

  // If distance is explicitly 0 but provider location is NOT missing, it's high confidence (service at shop or already on-site)
  // We only downgrade if distance is 0 AND we suspect it might be missing (base tier but providerBaseMissing is true)
  if (
    distanceKm <= 0 &&
    input.radiusTier === 'base' &&
    input.providerBaseMissing
  ) {
    return 'medium';
  }
...
export function calculatePricingQuote(
  ...
  const adjustmentMultiplier =
    COMPLEXITY_MULTIPLIER[jobComplexity] * URGENCY_MULTIPLIER[urgency];
  const laborAmount = roundMoney(
    input.pricingMode === 'hourly'
      ? providerPrice * hoursRequired
      : providerPrice,
  );
  const baselineMin = positiveNumber(
    input.laborBaseline?.minLaborAmount,
    laborAmount,
  );
  const baselineTypical = positiveNumber(
    input.laborBaseline?.typicalLaborAmount,
    laborAmount,
  );
  const baselineMax = positiveNumber(
    input.laborBaseline?.maxLaborAmount,
    laborAmount,
  );
  const fairLaborMin = roundMoney(baselineMin * adjustmentMultiplier);
  const benchmarkLaborAmount = roundMoney(
    baselineTypical * adjustmentMultiplier,
  );
  const fairLaborMax = roundMoney(
    Math.max(baselineMax, baselineTypical, baselineMin) * adjustmentMultiplier,
  );
  ...
  const geolocationDistanceKm = haversineKm(
    input.providerCoordinates,
    input.serviceCoordinates,
  );
  const oneWayDistanceKm = roundMoney(
    nonNegativeNumber(
      geolocationDistanceKm ?? input.distanceKm,
      fallbackTierKm,
    ),
  );
  const roundTripKm = roundMoney(
    oneWayDistanceKm * ROAD_DISTANCE_FACTOR * ROUND_TRIP_MULTIPLIER,
  );
  const travelAdjustment = roundMoney(
    roundTripKm > 0
      ? (roundTripKm / vehicle.fuelEfficiencyKmPerLiter) * fuelPrice
      : 0,
  );
  const distanceLaborAllowance = roundMoney(
    roundTripKm * DISTANCE_LABOR_ALLOWANCE_PER_KM,
  );
  const operatingBuffer = roundMoney(
    (benchmarkLaborAmount + travelAdjustment + distanceLaborAllowance) *
      OPERATING_BUFFER_RATE,
  );
  const fairMin = roundMoney(
    fairLaborMin + travelAdjustment + distanceLaborAllowance + operatingBuffer,
  );
  const fairEstimate = roundMoney(
    benchmarkLaborAmount +
      travelAdjustment +
      distanceLaborAllowance +
      operatingBuffer,
  );
  const fairMax = roundMoney(
    fairLaborMax + travelAdjustment + distanceLaborAllowance + operatingBuffer,
  );
  ...
  if (input.laborBaseline) {
    ...
    assumptions.push(
      'No category labor benchmark available; using provider labor price.',
    );
  }
  if (!input.vehicle) {
  ...
  if (oneWayDistanceKm > 0) {
    assumptions.push(
      `Travel uses ${oneWayDistanceKm.toFixed(2)} km one-way estimated distance and round-trip return assumption.`,
    );
  }
  if (input.fuel.freshness === 'stale') {
    assumptions.push(
