...
export class ProviderService {
  ...
  async getProviderReviews(providerId: string) {
    ...
    if (profileErr) {
      throw new InternalServerErrorException(`Failed to fetch provider profile stats: ${profileErr.message}`);
    }
    if (!profile) {
    ...
    if (reviewsErr) {
      throw new InternalServerErrorException(`Failed to fetch individual reviews: ${reviewsErr.message}`);
    }
    ...
    return {
      ...
      data: {
        ...
        reviews: reviews, // Array of all existing reviews
      }
    };
  }
  ...
  async getTrustScore(providerId: string) {
    ...
    if (error) {
      throw new InternalServerErrorException(`Failed to fetch trust score: ${error.message}`);
    }
    if (!data) {
      throw new NotFoundException('Provider profile not found');
    }
