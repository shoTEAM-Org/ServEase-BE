...
export class PaymentsService {
  ...
  async createPayment(dto: CreatePaymentDto) {
    ...
    if (insertError) throw new InternalServerErrorException(`Failed to process payment: ${insertError.message}`);
  ...
  async getEarnings(providerId: string) {
    if (!providerId) throw new BadRequestException('Provider ID is required to fetch earnings');
    ...
    if (fetchError) throw new InternalServerErrorException(`Error fetching earnings: ${fetchError.message}`);
