...
async function bootstrap(retryCount = 0) {
  try {
    ...
    if (retryCount < MAX_RETRIES) {
      console.warn(`Auth Service failed to start (attempt ${retryCount + 1}/${MAX_RETRIES}): ${error.message}`);
      console.warn(`Retrying in ${RETRY_DELAY_MS / 1000}s...`);
      await new Promise((resolve) => setTimeout(resolve, RETRY_DELAY_MS));
      return bootstrap(retryCount + 1);
    }
    console.error('Auth Service failed to start after max retries:', error);
    process.exit(1);
  }
}
bootstrap();
