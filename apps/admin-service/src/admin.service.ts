...
import { UpdateDocumentStatusDto } from '@app/common';
...
export class AdminService {
  ...
  async updateDocumentStatus(documentId: string, dto: UpdateDocumentStatusDto) {
    ...
    if (fetchError || !document) throw new NotFoundException(`Document with ID ${documentId} not found`);
    ...
    if (updateError) throw new BadRequestException(`Failed to update document status: ${updateError.message}`);
    ...
    if (profileError) console.error(`Error updating provider profile for ${providerId}:`, profileError);
    ...
    if (userError) console.error(`Error updating user status for ${providerId}:`, userError);
